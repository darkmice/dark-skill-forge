...
- **Fallback**：迁移或能力不足时的兼容路径，必须有范围、指标和退出条件。

不要按技术表象给 Adapter 排高低。页面事件、虚拟鼠标、人工流程或批处理只要真实承载业务执行并服从同一内核，就可以是一等 Projection。应分别审查“由谁决定下一步、多久重新规划”和“动作实际上怎样执行”，不能把控制循环低效误判成执行机制错误。
...
execution：通过 API、页面事件、鼠标、任务或人工怎样真正执行
presentation：用户如何看见、理解、暂停和接管
```

优化其中一层，不得无证据删除另外两层已有价值。例如虚拟鼠标可能是真实 UI 执行器；需要减少的可能是每个点击都重新调用模型，而不是点击本身。
...
- 先冻结事实基线和覆盖账本；
- 新路径以 feature detection/flag 渐进启用；
- 旧路径作为有指标的 fallback；
- 新路径真实 E2E 后才删除重复；
- fallback 没有退出条件时不得称为迁移完成。
