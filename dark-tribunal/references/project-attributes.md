...
- 审核轮中的 `checked` 只表示属性存在性、影响关系、代码路径和验证计划已审查；若需要运行时证据，是否实际通过只由阶段级 `stage-validation` 记录。
...
以下项目在项目属性审查时进入阶段验证矩阵；实际运行统一放在 Dark Tribunal 审核与集中修复关闭后的 `stage-validation`，不按 key、locale、文件或小修复重复执行。
...
第一轮从项目配置与 source 出发，确认属性存在性、实现表面及其与变更的关系，逐项审核命中属性的矩阵并登记阶段验证项。第二轮必须换到消费者或运行时方向，例如从渲染文本、错误出口、路由、后台任务、测试 fixture 或构建产物反查配置和 source。实际测试和运行验证仍在审核关闭后集中执行。
